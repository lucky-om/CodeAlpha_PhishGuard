...
import { QrCode, ShieldCheck, ShieldAlert, Scan, ExternalLink, X } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="space-y-8">
          ...
          <div className="relative w-full max-w-xs mx-auto group">
            <div className="p-8 bg-white rounded-3xl shadow-2xl border-4 border-slate-100 relative overflow-hidden select-none">
              <img
                src="/sample_phishing_qr_1775843127186.png"
                alt="Suspicious QR Code Simulation"
              ...
              {/* Glassmorphic text label */}
              <div className="absolute bottom-3 left-3 right-3 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-2xl px-4 py-2 text-center border border-slate-200">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Scan to Pay</span>
              </div>
            </div>
            ...
            {!isScanning && !scanResult && (
              <div className="absolute inset-0 flex items-end justify-center pb-4 opacity-0 group-hover:opacity-100 transition-opacity rounded-3xl pointer-events-none">
                <div className="pointer-events-auto">
                  <button
                    onClick={mockScan}
                    className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold flex items-center gap-2 shadow-2xl text-sm"
                  >
          ...
          {!scanResult && (
            <div className="flex justify-center">
              <button
                onClick={mockScan}
                disabled={isScanning}
                className="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-bold flex items-center gap-3 shadow-xl disabled:opacity-50 transition-all hover:scale-105"
              >
        ...
        <div className="space-y-6">
          {/* QR Beacon Info Panel */}
          <AnimatePresence mode="wait">
            {activeHotspot && qrHotspots.find(h => h.id === activeHotspot.id) && (
              <Motion.div
              ...
              >
                <div className={`px-6 py-5 flex items-start justify-between gap-4 ${colorMap[activeHotspot.color].bg}`}>
                  ...
                  <button onClick={() => setActiveHotspot(null)} className="p-1 rounded-lg hover:bg-black/10 transition-colors shrink-0">
            ...
            {scanResult && (
              ...
              >
                <div className="px-6 py-5 bg-rose-500/10 border-b border-rose-500/20 flex items-center justify-between">
                  ...
                  <button onClick={() => { setScanResult(null); setActiveHotspot(null); }} className="p-1 rounded-lg hover:bg-rose-500/10">
                ...
                <div className="p-6 space-y-5">
                    ...
                    {scanResultHotspots.map((spot) => {
                      ...
                      return (
                        <button
                          key={spot.id}
                          onClick={() => toggleHotspot(spot)}
                          className={`w-full text-left p-4 rounded-2xl border-2 transition-all flex items-start gap-3 ${isActive ? `${c.bg} ${c.ring}` : 'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 hover:border-slate-300'}`}
                        >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="py-3 px-4 bg-rose-500 text-white rounded-xl text-center font-black text-[10px] uppercase tracking-widest">MALICIOUS</div>
                    <div className="py-3 px-4 bg-slate-100 dark:bg-slate-800 text-slate-400 rounded-xl text-center text-[10px] flex items-center justify-center gap-1.5 font-bold opacity-40 cursor-not-allowed">
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-4 p-5 rounded-2xl bg-rose-500/5 border border-rose-500/20">
